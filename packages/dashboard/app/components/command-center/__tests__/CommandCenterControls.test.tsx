...
describe("CommandCenterControls concurrency markers", () => {
  ...
  it("keeps the Theme card's compact dropdown interactive", () => {
    ...
    const liquidGlassOption = screen.getByRole("option", { name: "Liquid Glass" });
    ...
    fireEvent.click(liquidGlassOption);
    ...
    const cozyOption = screen.getByRole("option", { name: "Cozy Cartoon" });
    expect(cozyOption.querySelector(".theme-swatch-cozy-cartoon")).toBeTruthy();
    fireEvent.click(cozyOption);

    expect(onColorThemeChange).toHaveBeenCalledWith("cozy-cartoon");
    expect(screen.queryByRole("listbox")).not.toBeInTheDocument();
  });
