 ...
 * Encryption at rest is deliberately declined for this same-UID local device
 * credential, like ~/.fusion/auth.json. SecretsStore uses PostgreSQL and
 * AES-256-GCM behind MasterKeyManager, but pairing must work before the engine
 * or database exists and on hosts that never run PostgreSQL; a same-UID reader
 * of this file can also read any local key used to wrap it.
 */
